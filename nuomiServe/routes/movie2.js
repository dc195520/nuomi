...
var fs=require('fs');
var msJson2 =require("../mocks/nuoMiMovieList");
...
router.get('/ms', function(req, res) {
    ...
    function deleteJson(id){
        ...
        for (var i = 0; i < datalist.length; i++) {
            if (id == datalist[i].movieId) {
                datalist.splice(i, 1);
            }
        }
        datalist.total = datalist.length;
        var str = JSON.stringify(datalist);
        fs.writeFile('../mocks/nuoMiMovieList.json',str,function(err){
            if(err){
                console.error(err);
            }
            console.log("----------删除成功------------");
...
router.post('/updatems',function(req,res) {
    ...
    function updateJson(list, ids) {
        fs.readFile('../mocks/nuoMiMovieList.json', function (err, data) {
            ...
            //把数据读出来更改
            for (var i = 0; i < datalist.length; i++) {
                if (ids == datalist[i].movieId) {
                    datalist[i].movieId=list.movieId;
                    datalist[i].movieName=list.movieName;
                    datalist[i].movieImg=list.movieImg;
                    datalist[i].movieScore=list.movieScore;
                    datalist[i].movieSumTime=list.movieSumTime;
                    datalist[i].movieShowTime=list.movieShowTime;
                }
            }
            var str = JSON.stringify(datalist);
            fs.writeFile('../mocks/nuoMiMovieList.json',str,function(err){
...
router.post("/delms",function (req,res) {
    ...
    list=JSON.parse(list)
    console.log(list)
    fs.readFile("../mocks/nuoMiMovieList.json","utf-8", function(err, data) {
